...
A[
    0
] = """综合运用Search,News,Finance工具完成以下单项选择题,最终回复明确选项,如1.A .你一定需要将题目和选项都作为root_node的content,避免遗漏信息.
在response节点中需要明确指出你的答案，如1.A  格式为题号：选项 不要输出具体该选项内容.
请注意WebSearchGraph类已经存在，请不要重新定义或from import,直接运用即可。
"""

# 打开并读取文本文件
with open("assets/newquestion.txt", "r", encoding="utf-8") as file:
...
# 打开一个文件用于写入答案
with open("assets/NewFinsearch.txt", "w", encoding="utf-8") as f:
