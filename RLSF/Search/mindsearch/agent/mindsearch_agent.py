        ...
        **kwargs,
        ...
        if parent_response:
            if "context" in self.template:
                parent_response = [
                    self.template["context"].format(**item) for item in parent_response
                ]
                message = "\n".join(parent_response + [message])
        print(colored(f"current query: {message}", "green"))
        for agent_return in super().stream_chat(
            message, session_id=random.randint(0, 999999), **kwargs
        ):
...
class FinanceSearchAgent(Internlm2Agent):
    def __init__(self, template="{query}", **kwargs) -> None:
        super().__init__(**kwargs)
        self.template = template
    ...
    def stream_chat(
        ...
        **kwargs,
        ...
        print(colored(f"current query: {message}", "cyan"))

        for agent_return in super().stream_chat(
            message, session_id=random.randint(0, 999999), **kwargs
        ):
            agent_return.type = "Finance"
            agent_return.content = message
            yield deepcopy(agent_return)


class NewsSearchAgent(Internlm2Agent):
    ...
    def stream_chat(
        ...
        **kwargs,
        ...
        print(colored(f"current query: {message}", "black"))

        for agent_return in super().stream_chat(
            message, session_id=random.randint(0, 999999), **kwargs
        ):
            agent_return.type = "News"
            agent_return.content = message
            yield deepcopy(agent_return)
...
class MindSearchProtocol(Internlm2Protocol):
    ...
    def format(
        self, inner_step: List[Dict], plugin_executor: ActionExecutor = None, **kwargs
    ) -> list:
        formatted = []
        if self.meta_prompt:
            formatted.append(dict(role="user", content=self.meta_prompt))
        if self.plugin_prompt:
            ...
            formatted.append(dict(role="user", content=plugin_prompt, name="plugin"))
        if self.interpreter_prompt:
            formatted.append(
                dict(role="user", content=self.interpreter_prompt, name="interpreter")
            )
        if self.few_shot:
...
# 加入对FinanceSearchAgent的支持，NewsSearchAgent同理
class WebSearchGraph:
    ...
    def add_node(self, node_name, node_content, node_type="searcher"):
        self.nodes[node_name] = dict(content=node_content, type=node_type)
        self.adjacency_list[node_name] = []
        def model_stream_thread():
            # 根据节点类型选择对应的 Agent
            if node_type == "searcher":
                agent = SearcherAgent(**self.searcher_cfg)
            elif node_type == "Finance":
                agent = FinanceSearchAgent(**self.finance_searcher_cfg)
            elif node_type == "News":
                agent = NewsSearchAgent(**self.news_searcher_cfg)
            else:
                agent = SearcherAgent(**self.searcher_cfg)  # 默认使用 SearcherAgent
            ...
            try:
                ...
                parent_response = [
                    dict(question=node["content"], answer=node.get("response", ""))
                ...
                for answer in agent.stream_chat(
                    ...
                    self.searcher_resp_queue.put(
                        deepcopy(
                            (
                                node_name,
                                dict(response=answer.response, detail=answer),
                                [],
                            )
                        )
                    )
        ...
        self.future_to_query[self.executor.submit(model_stream_thread)] = (
            f"{node_name}-{node_content}"
        )

    def add_response_node(self, node_name="response"):
        self.nodes[node_name] = dict(type="end", content="")
        self.searcher_resp_queue.put((node_name, self.nodes[node_name], []))
...
class MindSearchAgent(BaseAgent):
    def __init__(
        ...
        WebSearchGraph.finance_searcher_cfg = finance_searcher_cfg
        WebSearchGraph.news_searcher_cfg = news_searcher_cfg
    ...
    def _process_code(self,
        ...
        if node and 'response' in node and node['response']:
            inner_history.append({
                'role': 'environment',
                'content': str(node['response']),
                'name': re.sub(r'[^a-zA-Z0-9_-]', '_', node_name)
            })
        agent_return.inner_steps = deepcopy(inner_history)
